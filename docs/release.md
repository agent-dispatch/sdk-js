...
`@agent-dispatch/sdk` is published after `@agent-dispatch/core`.

## Prerequisites

- Publish `@agent-dispatch/core` for the target compatibility line.
- Configure npm Trusted Publisher for `agent-dispatch/sdk-js` using workflow `.github/workflows/publish.yml`.
